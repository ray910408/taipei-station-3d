...
2. 清單與找邊圖已隨 repo 提供（`rp/edge-list.json`、`rp/maps/edges-<樓層>.svg`）——正常情況直接用現成的。
   只有 nav 路網改動後才需要重產：`npm run gen:edges -- --svg --force`。**注意**：重產會更換清單的
   `generated` 版本戳，所有進行中的走線 session 將無法續採（守門會擋），請先收完再重產。
   把清單傳進手機，把找邊圖印出或存到手機。存手機建議放獨立資料夾並保留 `rp/maps/` 附帶的 `.nomedia`；SVG 會被部分相簿當圖片索引。
...
| `walkEnd` 警告「步數×步長與邊長差很大」，但確實走完全程 | 部分機型（實測 Samsung）硬體步偵測器在短邊／慢走時會大量漏步，甚至 0 步 | **資料仍可用**：原始 acc 已全程入檔，離線會用 acc 重建步事件做弧長錨定。照常走下一條即可；連續勻速走（不中途停頓）可改善硬體偵測 |
...
| 手機相簿部分照片載不出來/變慢 | 相簿把 `maps` 的 SVG 當圖片索引，舊版圖檔缺尺寸屬性會噎死縮圖器 | 更新後圖檔已帶尺寸＋`.nomedia`；仍出狀況就刪手機上的 `maps` 資料夾（app 運作只需要 `edge-list.json` / `rp-points.json`） |
