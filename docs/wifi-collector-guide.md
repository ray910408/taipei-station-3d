...
1. 產 RP 清單與找點圖(現行設定,187 點 / 約 2.7 小時):

   ```
   npm run gen:rp -- --spacing "12,tra-platform-b2=20,mrt-r-platform-b4=20" --n 5 --svg
   ```
   ...
   **為什麼是 12m / 月台 20m / N=5**(2026-07-29 北車試線實測,別再往下加密):
   - 使用者端掃一次的雜訊是 **10 dB**(手機不會開開發者選項關節流,走路中只拿得到一次掃描);
     而 2m 的空間差只有 8.5 dB——採得再密,使用者端也分不出來。
   - 指紋距離在 **4~6m 就飽和**(超過 6m 全部打平在 ~20 dB)。間距超過 ~12m,
     查詢點會落進「離每個 RP 都超過 6m」的死區,定位改用跳的而非漸變。12m 的中點剛好踩在邊緣。
   - N=5 的指紋庫雜訊 1.2 dB,遠低於使用者端的 10 dB;做到 N=10(0.9 dB)只是多花一倍時間。
   - 月台是長條一維空間、PDR 在那裡最準,20m 足夠。
