...
1. 產 RP 清單與找點圖(現行設定,171 點 / 約 2.5 小時):
...
| 「不能續採:清單版本不符」 | 該 session 是用舊清單開的,而重產的清單會把同一個點號指到不同座標——續採會靜默跳過不相干的新位置。**開新 session**;舊 session 的資料仍在,離線 pipeline 會依各自的 `rpGenerated` 分開處理 |
