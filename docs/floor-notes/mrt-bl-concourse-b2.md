...
- B1 全層為非付費區，因此「市民大道方向」兩梯下抵 B2 的實測豎井點
  `[85.5,-84.3]`、`[98.3,-89.0]` 改納入 `a-bc-unpaid-link`；兩點同時保留於 tc/bc
  floor data，供 Task 7 垂直 connectors 對齊。
- `a-bc-paid` 於 stem 北端沿口袋外緣內縮；`n-bc-010=[92.9,-81.4]` 仍在付費區，
  且付費路徑改經 `n-bc-017` 繞過非付費口袋後向南連通。
- 新增 `g-bc-link-in/out/acc` 與對應 gate edges；資訊圖未繪該閘門群，票務邊界、
  單向配置與無障礙道皆為 confidence 2 的語意推定。
