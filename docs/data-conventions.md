...
切線由兩段半弦估得 N14.8°（台大醫院→台北車站，L1=552.5 m）與 N21.0°（台北車站→中山，
L2=701.2 m）按弦長加權內插（見下）得 N17.49°
→ local +Y 與真北差 **0.02°**。

誤差來自參考點本身：`exit-coords.csv` 只有出入口座標，**站的「位置」只能取出口重心，而重心不是
軌道上的點**。推導時有三個地方容易算錯，都踩過：

1. **方位角只吃「垂直於弦」的那一半誤差。** 出口離散度 RMS（台北車站 129 m／8 個出口、
   台大醫院 105 m／4 個、中山 85 m／6 個）是二維徑向量，直接當成一維法向分量會高估。
   把各站出口的共變異數矩陣投影到該弦的法向、再除以出口數，得法向重心 SE
   15.4 m（台大醫院@弦1）、27.6／26.2 m（台北車站@弦1／弦2）、33.7 m（中山@弦2）——
   台大醫院差最多（15.4 vs 徑向 52.7），因為它的出口幾乎排在弦的方向上。
   各半弦 SE 因此是 **±3.3°／±3.5°**，不是徑向算出來的 ±7.2°／±4.7°。
2. **兩條半弦共用台北車站這個端點**，且它在兩式中以相反符號進入
   （δθ₁=(b⊥−a⊥)/L₁、δθ₂=(c⊥−b⊥)/L₂），平均值的變異數要含這項共變異，
   得 **±1.65°**；當成獨立來平均（不含共變異）會算成 ±2.39°，
   若連第 1 點的投影都跳過、直接拿徑向 RMS 當獨立弦平均，則到 ±4.3°。
3. **弦向是弧中點的切線，等權平均不是台北車站處的切線。** R 線有曲率，弦1
   （台大醫院→台北車站）中點在站南 ~276 m、弦2（台北車站→中山）中點在站北 ~351 m；
   等權平均 (θ1+θ2)/2 估的其實是站北 (L2−L1)/4 ≈ 37 m 處的切線（N17.86°），不是站處。
   等曲率下切線隨弧長線性變化，站處切線＝按弦長線性內插：
   θw=(θ1·L2+θ2·L1)/(L1+L2)，權重 w1=L2/(L1+L2)=0.559、w2=L1/(L1+L2)=0.441——
   得 **N17.49°**。誤差傳遞用同一組權重（δθw = w1·δθ1 + w2·δθ2），
   第 2 點的相關修正、獨立平均兩個 SE 也都改用這組權重算，數值幾乎不變（±1.65°／±2.39°）。

**結論：local +Y 與真北差 0.02°±1.65°（僅隨機項）。** 這個 0.02° 的吻合遠小於隨機項本身，
也遠小於下面的未量化系統性偏差，是巧合等級的精度——**結論仍是「未被推翻、不足以升級」**，
不能拿這個小差值反過來宣稱實測精度。

護欄容差仍取 6°，而且現在餘裕幾乎全部是留給**那條式子涵蓋不到的系統性偏差**：
台北車站（BL12/R10）與中山（R11/G14）都是轉乘站，出口分佈涵蓋兩條線的站體，
重心會被非 R 線那半邊拉走，而 CSV 沒有逐出口的線別可供拆分——那部分不是隨機誤差，
開不出標準差，也正是為什麼隨機項小到 ±1.65° 仍不足以宣告實測。長基線的弦
（台大醫院→雙連 14.0°／1793 m、台大醫院→民權西路 8.8°／2307 m）SE 較小，
但 R 線本身有曲率，弦不等於台北車站處的切線。

所以 `bearing_status` 維持 `estimated`，由 `tests/frame-bearing.test.ts` 當一致性護欄。
該測試也把三站的出口數（4／8／6）釘住——少一筆出口不會讓 bearing 斷言變紅，
但上面那段推導就不是照這些點算出來的了。更進一步，整條推導鏈（弦長、弦方位、
各站法向重心 SE、半弦 SE、相關修正後與獨立平均兩個對照值、徑向 RMS）都在該測試裡
重算並釘成斷言——出口座標若等量反向移動，重心、出口數、bearing 斷言都不會變，
但共變異矩陣會變，上面這串數字就會無聲過期；那條測試紅了就是訊號，要回去重推、
同步這裡的數字。
...
「底圖北方偏 local 北方約 6.5°」。照它轉，月台會變成 N24–26°E，遠在隨機項 ±1.65°（僅隨機項，見上）的
3.9–5.1σ 之外，且與長基線弦（8.8–14.0°）的走勢相反。底圖是示意圖，用來描相對形狀就好。
