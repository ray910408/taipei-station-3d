...
### 方位角（`bearing_status: crosschecked`）
`local +Y = 真北` 這件事已用北捷開放資料 `trtc-od-exit-coords` 核對過，**結論是原本的假設站得住**：
模型 `a-rp-platform` 主軸距 local +Y 為 17.5°，而由出入口 GPS 重心推得的實際 R 線走向為
N15.6–21.0°（台大醫院→中山 18.2°、兩段半弦平均 17.9°）——兩者差 0°±3°。
由 `tests/frame-bearing.test.ts` 守住；`bearing_deg` 若被改動、或月台幾何被整體旋轉，該測試會紅。

**不要改用底圖的指北針來校正。** trtc 系列四張圖的指北針主軸量得 12.3–14.3°，
以出入口 GPS 對 floor-1 做相似變換也得 14.7°（RMS 23.6 m，留一法 sd 1.0°）——
兩者都指向「底圖北方比 local 北方偏東約 6.5°」，也就是
`docs/floor-notes/mrt-r-platform-b4.md` 早就記下的那個繪製偏差。順著底圖去轉模型，
會讓月台變成 N24–26°E，反而離實際的 N15.6–21.0° 更遠。底圖是示意圖，真北以 GPS 為準。
