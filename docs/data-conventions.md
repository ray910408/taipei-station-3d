...
2. **兩條半弦共用台北車站這個端點**，且它在兩式中以相反符號進入
   ...
   得 **±1.6°**；當成獨立來平均（不含共變異）會算成 ±2.4°，
   若連第 1 點的投影都跳過、直接拿徑向 RMS 當獨立弦平均，則到 ±4.3°。

**結論：local +Y 與真北差 0.35°±1.6°（僅隨機項）。**
...
開不出標準差，也正是為什麼隨機項小到 ±1.6° 仍不足以宣告實測。長基線的弦
...
「底圖北方偏 local 北方約 6.5°」。照它轉，月台會變成 N24–26°E，遠在隨機項 ±1.6°（僅隨機項，見上）的
3.7–5.0σ 之外，且與長基線弦（8.8–14.0°）的走勢相反。底圖是示意圖，用來描相對形狀就好。
