...
軌道上的點**。各站出口離散度 RMS 台北車站 129 m、台大醫院 105 m、中山 85 m，重心標準誤 50–70 m，
換算到 552 m／701 m 的半弦就是 ±7.2°／±4.7°，平均後 **±4.3°**。而且台北車站（BL12/R10）與
中山（R11/G14）都是轉乘站，出口分佈涵蓋兩條線的站體，重心會被非 R 線的那半邊拉走——這個偏差
無法從這份 CSV 拆開（沒有逐出口的線別欄位）。長基線的弦（台大醫院→雙連 14.0°／1793 m、
台大醫院→民權西路 8.8°／2307 m）SE 較小，但 R 線本身有曲率，弦不等於台北車站處的切線。

結論：這條路能證明的是「0° 附近，±4~5°」，不足以宣告實測，所以 `bearing_status` 維持 `estimated`。
