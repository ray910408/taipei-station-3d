...
每列欄數（截斷的列不丟掉、直接當異常）、五檔站名集合是否對齊 118 站、每列 zip 的欄位個數、
編號集合（兩邊的值都寫進簽章）、複合列的對位順序、同編號重複、出口編號 join 得回不去、
座標值域（抓掉小數點那種壞值）、異體字，全部逐項檢查——上面六筆一筆不多一筆不少。以前分次手掃時 3、4 靠集合比對找到、卻看不到 5（集合相同只有順序反）；
