...
這份清單不是人工盤點出來的，是 `tests/opendata-joinkeys.test.ts` 掃出來的：
五檔的標頭欄名與列數（凍結在測試裡，不是拿進來的檔案自己的標頭當標準——整份一起少一欄
時那樣看是自洽的）、每列欄數（截斷的列不丟掉、直接當異常）、五檔站名集合是否對齊 118 站、
每列 zip 的欄位個數、兩檔的每站編號集合雙向比對、複合列的對位順序、同編號重複、
出口編號 join 得回不去、座標值域（抓掉小數點那種壞值）、異體字，全部逐項檢查——
上面六筆一筆不多一筆不少。重新下載後測試轉紅是正常的：回來核對差異、更新這裡與 EXPECTED。以前分次手掃時 3、4 靠集合比對找到、卻看不到 5（集合相同只有順序反）；
補上對位比對後又要小心別把「編號值本身就錯」的兩站誤報成順序錯——測試裡的順序檢查因此只在
兩檔編號集合一致時才判定。
